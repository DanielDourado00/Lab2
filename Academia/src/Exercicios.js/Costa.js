...
const initialBackExercises = [
    ...
    'Remada Sentada (Cabo) - 3 séries de 12 repetições',
    'Levantamento Terra - 3 séries de 6-8 repetições',
    'Pulldown Pullover no Cabo - 3 séries de 12 repetições',
];

export default function TelaInicio() {
    const [activeExercises, setActiveExercises] = useState(
        initialBackExercises.map((name, index) => ({ name, index }))
    );
    ...
    useEffect(() => {
        if (activeExercises.length === 0 && completedExercises.length === initialBackExercises.length) {
